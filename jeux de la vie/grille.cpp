...
Grille::Grille(int lignes, int colonnes) : lignes(lignes), colonnes(colonnes) {
    ...
    cellulesImmortelles = new bool[lignes * colonnes](); // Tout initialis� � false
    cellulesIndestructibles = new bool[lignes * colonnes](); // Tout initialis� � false
}
...
int Grille::verifierVoisins(int x, int y) {
    ...
    // V�rification des 8 voisins individuellement
    int voisinages[8][2] = {
        {-1, -1}, {-1, 0}, {-1, 1},  // ligne haut
        {0, -1},          {0, 1},   // ligne milieu (� gauche et � droite)
        {1, -1},  {1, 0}, {1, 1}   // ligne bas
    };

    // V�rifie chaque voisin
    for (int i = 0; i < 8; i++) {
...
void Grille::definirCelluleIndestructible(int x, int y) {
    if (x >= 0 && x < lignes && y >= 0 && y < colonnes) {
        ...
        cellulesIndestructibles[index] = !cellulesIndestructibles[index]; // Basculer l'�tat
...
void Grille::exporterGrille(const std::string& cheminFichier) const {
    ...
    fichier.close();
    std::cout << "Exportation termin�e vers " << cheminFichier << ".\n";
}
...
void Grille::definirCelluleImmortelle(int x, int y) {
    if (x >= 0 && x < lignes && y >= 0 && y < colonnes) {
        int index = x * colonnes + y;
        cellulesImmortelles[index] = !cellulesImmortelles[index]; // Basculer l'�tat
...
bool Grille::revenirEnArriere() {
    if (historique.empty()) {
        std::cerr << "Aucun �tat pr�c�dent disponible." << std::endl;
        return false;
    }

    delete[] tableau; // Lib�rer l'�tat actuel
    tableau = historique.top(); // R�cup�rer l'�tat pr�c�dent
    historique.pop(); // Supprimer l'�tat de la pile
    return true;
}

void Grille::mettreAJour() {
    sauvegarderEtat(); // Sauvegarder l'�tat actuel avant de le modifier
...
void Grille::changerCase(sf::RenderWindow& window, float cellSize, sf::Event& event, sf::Vector2f grilleOffset) {
    ...
    if (col >= 0 && col < colonnes && row >= 0 && row < lignes) {
        ...
        // V�rifier si la cellule est modifiable
        if (!cellulesImmortelles[index] && !cellulesIndestructibles[index]) {
            tableau[index] = !tableau[index];
            std::cout << "Case normale modifi�e : (" << row << ", " << col << ") -> "
                << (tableau[index] ? "vivante" : "morte") << std::endl;
        }
        else {
            std::cout << "Case prot�g�e (immortelle ou indestructible) : (" << row << ", " << col << ")\n";
        }
    }
}
